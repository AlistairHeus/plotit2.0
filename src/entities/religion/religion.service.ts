...
export class ReligionService {
    ...
    constructor(religionRepository: ReligionRepository, fileService: IFileService) {
        this.religionRepository = religionRepository;
        this.fileService = fileService;
    }

    async createReligion(data: CreateReligion, files?: Record<string, Express.Multer.File[]>): Promise<Religion> {
        if (files) {
            if (files.avatar && files.avatar.length > 0 && files.avatar[0]) {
                const avatarPath = await this.fileService.save(files.avatar[0], "religion");
    ...
    async updateReligion(id: string, data: UpdateReligion, files?: Record<string, Express.Multer.File[]>): Promise<Religion> {
        if (files) {
            if (files.avatar && files.avatar.length > 0 && files.avatar[0]) {
                const avatarPath = await this.fileService.save(files.avatar[0], "religion");
                data.avatarUrl = this.fileService.getUrl(avatarPath);
