...
export class MapRepository {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new DatabaseError(
              "Failed to find maps with relations",
              new Error(String(error)),
            ),
      };
    }
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new DatabaseError("Failed to find map", new Error(String(error))),
      };
    }
  }
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new DatabaseError(
              "Failed to find map with relations",
              new Error(String(error)),
            ),
      };
    }
  ...
  async upsertSvgMapping(data: CreateSvgMapping): Promise<Result<SvgMapping>> {
    try {
      ...
      if (!result) {
        return {
          success: false,
          error: new DatabaseError("Failed to upsert svg mapping"),
        };
      }
      return { success: true, data: result };
    } catch (error) {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new DatabaseError(
              "Failed to delete map svg mapping",
              new Error(String(error)),
            ),
      };
    }
  ...
  async removeSvgMappingByElementId(
    mapId: string,
    svgElementId: string,
    ...
    try {
      const [result] = await db
        ...
        .where(
          and(
            eq(mapSvgMappings.mapId, mapId),
            eq(mapSvgMappings.svgElementId, svgElementId),
          ),
        )
        .returning({ id: mapSvgMappings.id });
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new DatabaseError(
              "Failed to delete svg mapping by element",
              new Error(String(error)),
            ),
      };
    }
