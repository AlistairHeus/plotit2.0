...
function buildWhereConditions(queryParams: PowerSystemQueryParams): SQL[] {
  ...
  if (
    "rootOfPowerId" in queryParams &&
    typeof queryParams.rootOfPowerId === "string"
  ) {
    whereConditions.push(
      eq(powerSystems.rootOfPowerId, queryParams.rootOfPowerId),
    );
  }
...
export class PowerSystemRepository {
  ...
  async createRoot(data: CreateRootOfPower): Promise<Result<RootOfPower>> {
    try {
      ...
      if (!result)
        return {
          success: false,
          error: new DatabaseError("Failed to create rootOfPower"),
        };
  ...
  async updateRoot(
    id: string,
    data: UpdateRootOfPower,
  ): Promise<Result<RootOfPower>> {
    try {
      const [result] = await db
        .update(rootsOfPower)
        .set({ ...data, updatedAt: new Date() })
        .where(eq(rootsOfPower.id, id))
        .returning();
      if (!result)
        return { success: false, error: new NotFoundError("RootOfPower", id) };
      return { success: true, data: result };
    } catch (error) {
      return {
        success: false,
  ...
  async createPowerSystem(
    data: CreatePowerSystem,
  ): Promise<Result<PowerSystem>> {
    try {
  ...
  async findOnePowerSystemWithRelations(
    id: string,
  ): Promise<Result<PowerSystemWithRelations>> {
    try {
      const result = await db.query.powerSystems.findFirst({
        where: eq(powerSystems.id, id),
        with: {
  ...
  async createSubSystem(
    data: CreatePowerSubSystem,
  ): Promise<Result<PowerSubSystem>> {
    try {
      const [result] = await db
        .insert(powerSubSystems)
        .values(data)
        .returning();
      if (!result)
        return {
          success: false,
          error: new DatabaseError("Failed to create powerSubSystem"),
        };
      return { success: true, data: result };
    } catch (error) {
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new DatabaseError(
                "Failed to create powerSubSystem",
                new Error(String(error)),
              ),
      };
    }
  ...
  async updateSubSystem(
    id: string,
    data: UpdatePowerSubSystem,
  ): Promise<Result<PowerSubSystem>> {
    try {
      const [result] = await db
        .update(powerSubSystems)
        .set({ ...data, updatedAt: new Date() })
        .where(eq(powerSubSystems.id, id))
        .returning();
      if (!result)
        return {
          success: false,
          error: new NotFoundError("PowerSubSystem", id),
        };
      return { success: true, data: result };
    } catch (error) {
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new DatabaseError(
                "Failed to update powerSubSystem",
                new Error(String(error)),
              ),
      };
    }
  ...
  async deleteSubSystem(id: string): Promise<Result<boolean>> {
    try {
      const [result] = await db
        .delete(powerSubSystems)
        .where(eq(powerSubSystems.id, id))
        .returning({ id: powerSubSystems.id });
      return { success: true, data: result !== undefined };
    } catch (error) {
      return {
        success: false,
        error:
          error instanceof Error
            ? error
            : new DatabaseError(
                "Failed to delete powerSubSystem",
                new Error(String(error)),
              ),
      };
    }
  ...
  async createCategory(
    data: CreatePowerCategory,
  ): Promise<Result<PowerCategory>> {
    try {
      const [result] = await db
        .insert(powerCategories)
        .values(data)
        .returning();
      if (!result)
        return {
          success: false,
          error: new DatabaseError("Failed to create powerCategory"),
        };
      return { success: true, data: result };
    } catch (error) {
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new DatabaseError(
                "Failed to create powerCategory",
                new Error(String(error)),
              ),
      };
    }
  ...
  async updateCategory(
    id: string,
    data: UpdatePowerCategory,
  ): Promise<Result<PowerCategory>> {
    try {
      const [result] = await db
        .update(powerCategories)
        .set({ ...data, updatedAt: new Date() })
        .where(eq(powerCategories.id, id))
        .returning();
      if (!result)
        return {
          success: false,
          error: new NotFoundError("PowerCategory", id),
        };
      return { success: true, data: result };
    } catch (error) {
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new DatabaseError(
                "Failed to update powerCategory",
                new Error(String(error)),
              ),
      };
    }
  ...
  async deleteCategory(id: string): Promise<Result<boolean>> {
    try {
      const [result] = await db
        .delete(powerCategories)
        .where(eq(powerCategories.id, id))
        .returning({ id: powerCategories.id });
      return { success: true, data: result !== undefined };
    } catch (error) {
      return {
        success: false,
        error:
          error instanceof Error
            ? error
            : new DatabaseError(
                "Failed to delete powerCategory",
                new Error(String(error)),
              ),
      };
    }
  ...
  async createAbility(data: CreatePowerAbility): Promise<Result<PowerAbility>> {
    try {
      ...
      if (!result)
        return {
          success: false,
          error: new DatabaseError("Failed to create powerAbility"),
        };
      return { success: true, data: result };
    } catch (error) {
      return {
  ...
  async updateAbility(
    id: string,
    data: UpdatePowerAbility,
  ): Promise<Result<PowerAbility>> {
    try {
      const [result] = await db
        .update(powerAbilities)
        .set({ ...data, updatedAt: new Date() })
        .where(eq(powerAbilities.id, id))
        .returning();
      if (!result)
        return { success: false, error: new NotFoundError("PowerAbility", id) };
      return { success: true, data: result };
    } catch (error) {
      return {
        success: false,
  ...
  async deleteAbility(id: string): Promise<Result<boolean>> {
    try {
      const [result] = await db
        .delete(powerAbilities)
        .where(eq(powerAbilities.id, id))
        .returning({ id: powerAbilities.id });
      return { success: true, data: result !== undefined };
    } catch (error) {
      return {
        success: false,
