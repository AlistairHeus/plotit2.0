...
export class PowerSystemService {
  ...
  async getPowerSystems(
    ...
    const result =
      await this.powerSystemRepository.findAllPowerSystems(queryParams);
    if (!result.success) throw result.error;
    return result.data;
  }
  ...
  async getPowerSystemById(id: string): Promise<PowerSystem | null> {
    const result =
      await this.powerSystemRepository.findOnePowerSystemWithRelations(id);
