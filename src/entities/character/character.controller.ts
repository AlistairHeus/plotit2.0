...
export class CharacterController {
  ...
  async getPowerAccess(req: Request, res: Response): Promise<void> {
    ...
    log.info("Character power access retrieved", {
      characterId,
      operation: "get_power_access",
    });
    res
      .status(200)
      .json({
        success: true,
        data: powers,
        message: "Power access retrieved successfully",
      });
  }
  ...
  async syncPowerAccess(req: Request, res: Response): Promise<void> {
    ...
    const powers = await this.characterService.syncPowerAccess(
      characterId,
      data,
    );

    log.info("Character power access synced", {
      characterId,
      operation: "sync_power_access",
    });
    res
      .status(200)
      .json({
