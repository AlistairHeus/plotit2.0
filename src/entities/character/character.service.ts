...
export class CharacterService {
    ...
    constructor(characterRepository: CharacterRepository, fileService: IFileService) {
        this.characterRepository = characterRepository;
        this.fileService = fileService;
    }

    async createCharacter(data: CreateCharacter, files?: Record<string, Express.Multer.File[]>): Promise<Character> {
        if (files) {
            if (files.avatar && files.avatar.length > 0 && files.avatar[0]) {
                const avatarPath = await this.fileService.save(files.avatar[0], "character");
    ...
    async updateCharacter(id: string, data: UpdateCharacter, files?: Record<string, Express.Multer.File[]>): Promise<Character> {
        if (files) {
            if (files.avatar && files.avatar.length > 0 && files.avatar[0]) {
                const avatarPath = await this.fileService.save(files.avatar[0], "character");
                data.avatarUrl = this.fileService.getUrl(avatarPath);
