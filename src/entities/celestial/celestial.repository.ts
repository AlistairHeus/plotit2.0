...
function buildGalaxyWhereConditions(queryParams: GalaxyQueryParams): SQL[] {
  ...
  if (queryParams.name)
    whereConditions.push(eq(galaxies.name, queryParams.name));
  ...
  async update(id: string, data: UpdateGalaxy): Promise<Result<Galaxy>> {
    try {
      const [result] = await db
        ...
        .returning();
      if (!result)
        return { success: false, error: new NotFoundError("Galaxy", id) };
      return { success: true, data: result };
    } catch (error) {
      return {
        success: false,
...
function buildSolarSystemWhereConditions(
  ...
  async update(id: string, data: UpdateStar): Promise<Result<Star>> {
    try {
      const [result] = await db
        ...
        .returning();
      if (!result)
        return { success: false, error: new NotFoundError("Star", id) };
      return { success: true, data: result as Star };
    } catch (error) {
      return {
        success: false,
...
function buildPlanetWhereConditions(queryParams: PlanetQueryParams): SQL[] {
  const whereConditions: SQL[] = [];
  if (queryParams.name)
    whereConditions.push(eq(planets.name, queryParams.name));
  if (queryParams.systemId)
    whereConditions.push(eq(planets.systemId, queryParams.systemId));
  if (queryParams.parentPlanetId)
    whereConditions.push(
      eq(planets.parentPlanetId, queryParams.parentPlanetId),
    );
  ...
  async update(id: string, data: UpdatePlanet): Promise<Result<Planet>> {
    try {
      const [result] = await db
        ...
        .returning();
      if (!result)
        return { success: false, error: new NotFoundError("Planet", id) };
      return { success: true, data: result as Planet };
    } catch (error) {
      return {
        success: false,
