...
export class CelestialService {
  ...
  ): Promise<PaginatedResponse<Galaxy>> {
  ...
  async createPlanet(
    ...
    if (data.parentPlanetId) {
      const parentResult = await this.planetRepository.findOne(
        data.parentPlanetId,
      );
      if (parentResult.success) {
        if (parentResult.data.systemId !== data.systemId) {
          throw new InvalidArgumentError(
            "Moon's systemId must match parent planet's systemId",
          );
        }
      }
    }
  ...
  async updatePlanet(
    ...
    if (data.parentPlanetId) {
      ...
      if (planetResult.success) {
        ...
        if (parentResult.success) {
          const systemId =
            "systemId" in data && data.systemId
              ? data.systemId
              : planetResult.data.systemId;
