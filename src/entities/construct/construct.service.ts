...
export class ConstructService {
    ...
    constructor(constructRepository: ConstructRepository, fileService: IFileService) {
        this.constructRepository = constructRepository;
        this.fileService = fileService;
    }

    async createConstruct(data: CreateConstruct, files?: Record<string, Express.Multer.File[]>): Promise<Construct> {
        if (files) {
            if (files.avatar && files.avatar.length > 0 && files.avatar[0]) {
                const avatarPath = await this.fileService.save(files.avatar[0], "construct");
    ...
    async updateConstruct(id: string, data: UpdateConstruct, files?: Record<string, Express.Multer.File[]>): Promise<Construct> {
        if (files) {
            if (files.avatar && files.avatar.length > 0 && files.avatar[0]) {
                const avatarPath = await this.fileService.save(files.avatar[0], "construct");
                data.avatarUrl = this.fileService.getUrl(avatarPath);
