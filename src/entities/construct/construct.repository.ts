...
function buildWhereConditions(queryParams: ConstructQueryParams): SQL[] {
    ...
    if ("category" in queryParams && queryParams.category) {
        whereConditions.push(eq(constructs.category, queryParams.category as ConstructCategory));
    }
            ...
            return {
                ...
                error:
                    error instanceof Error
                        ? error
                        : new DatabaseError("Failed to create construct", new Error(String(error))),
            };
        }
    }
        ...
        try {
            ...
            return {
                ...
                error:
                    error instanceof Error
                        ? error
                        : new DatabaseError("Failed to update construct", new Error(String(error))),
            };
        }
    }
        ...
        try {
            ...
            return {
                ...
                error:
                    error instanceof Error
                        ? error
                        : new DatabaseError("Failed to delete construct", new Error(String(error))),
            };
        }
    }
        ...
        try {
            ...
            return {
                ...
                error:
                    error instanceof Error
                        ? error
                        : new DatabaseError("Failed to find constructs", new Error(String(error))),
            };
        }
    }
        ...
        try {
            ...
            return {
                ...
                error:
                    error instanceof Error
                        ? error
                        : new DatabaseError("Failed to find construct", new Error(String(error))),
            };
        }
    }

    async findOneWithRelations(id: string): Promise<Result<ConstructWithRelations>> {
        try {
            const result = await db.query.constructs.findFirst({
                where: eq(constructs.id, id),
                with: {
